...
int rat_get_remap(RAT *rat, int arf_id)
{
    ...
    for(int i = 0; i < MAX_ARF_REGS; i++)
    {
        if(rat->entries[i].valid && (rat->entries[i].prf_id == (uint64_t)arf_id))
            return rat->entries[i].prf_id;
    }
...
void rat_set_remap(RAT *rat, int arf_id, int prf_id)
{
    ...
    for(int i = 0; i < MAX_ARF_REGS; i++)
    {
        if(rat->entries[i].valid && (rat->entries[i].prf_id == (uint64_t)arf_id))// doubt - verify later
            rat->entries[i].prf_id = prf_id;
    }
    */
    rat->entries[arf_id].valid = true;
    rat->entries[arf_id].prf_id = (uint64_t)prf_id;
}
