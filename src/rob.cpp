...
bool rob_check_space(ROB *rob)
{
    ...
    /*if(rob->tail_ptr != rob->head_ptr)
        return true;

    return false;*/
    
    if(rob->head_ptr == rob->tail_ptr and rob->entries[rob->tail_ptr].valid)
        return false;
...
int rob_insert(ROB *rob, InstInfo inst)
{
    ...
    rob->entries[rob->tail_ptr].inst = inst; // check rob->tail_ptr
    rob->entries[rob->tail_ptr].valid = true;
    int ind = rob->tail_ptr; // getting the index of rob to which the instruction is inserted before updating the tail_ptr
    if(rob->tail_ptr + 1 == (int)NUM_ROB_ENTRIES)
        rob->tail_ptr = 0;
    else
        rob->tail_ptr++;
...
void rob_mark_exec(ROB *rob, InstInfo inst)
{
    ...
    if(rob->head_ptr < rob->tail_ptr) // if entries in rob is within the max limit
    {
        for( int i = rob->head_ptr; i < rob->tail_ptr; i++)
        {
    ...
    else if(rob->head_ptr >= rob->tail_ptr) // if entries in rob is beyond the max limit
    {
        for(unsigned int i = rob->head_ptr; i < NUM_ROB_ENTRIES; i++)
        {
            if(rob->entries[i].valid && inst.inst_num == rob->entries[i].inst.inst_num)
            {
                rob->entries[i].exec = true;
            }
        }

        for(int i = 0; i < rob->tail_ptr; i++)
        {
            if(rob->entries[i].valid && inst.inst_num == rob->entries[i].inst.inst_num)
            {
                rob->entries[i].exec = true;
...
bool rob_check_head(ROB *rob)
{
    ...
    if(rob->entries[rob->head_ptr].valid and rob->entries[rob->head_ptr].ready)
        return true;
...
InstInfo rob_remove_head(ROB *rob)
{
    ...
    if(rob->entries[rob->head_ptr].ready)
    {
        ...
        if(rob->head_ptr + 1 == (int)NUM_ROB_ENTRIES){
            rob->head_ptr = 0;
        }
        else{
            rob->head_ptr++;
